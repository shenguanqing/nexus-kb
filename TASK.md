...
- [ ] 引入 Prisma 与不可变迁移。
- [ ] 定义 Document、DocumentVersion、IngestionJob 模型及 tenant 索引。
- [ ] 引入 BullMQ，使用 ingestionJobId 作为稳定 jobId。
- [ ] 保存任务状态、尝试次数、trace ID、parser 版本和 warning。
- [ ] 增加 PostgreSQL/Redis 集成测试。

### 3.2 安全上传

- [ ] 实现流式上传与大小限制。
- [ ] 白名单化 TXT、Markdown、DOCX、XLSX。
- [ ] 联合校验扩展名、MIME 与 magic bytes。
- [ ] 使用 UUID 存储文件并计算 SHA-256。
- [ ] 拒绝路径穿越、软链接、空文件、伪造 MIME 和超限文件。
- [ ] tenant、department、owner 和 sensitivity 只来自服务端身份/策略。

### 3.3 文档 API

- [ ] `POST /v1/documents` 返回 `202`、documentId 和 jobId。
- [ ] `GET /v1/documents/:documentId` 强制 tenant 过滤。
- [ ] `GET /v1/ingestion-jobs/:jobId` 强制 tenant 过滤。
- [ ] `DELETE /v1/documents/:documentId` 先实现安全、幂等的状态与原文件删除。
- [ ] 错误响应包含稳定 code 和 trace ID，不泄露存储路径。

### 3.4 第一批解析器与契约

- [ ] 将 TXT/Markdown 从单元素存根升级为保留标题路径的结构化元素。
- [ ] 实现 DOCX 解析器。
- [ ] 实现 XLSX/openpyxl 解析器并保留 sheet 与表头。
- [ ] 加入 MIME/后缀路由和空结果失败策略。
- [ ] 配置解析超时、页数/行数与结果元素数量限制。
- [ ] 增加真实 Worker 契约测试，覆盖缺字段、错误类型和超大响应。

### 3.5 安全与端到端测试

- [ ] 两个 tenant 无法读取彼此文档或任务。
- [ ] 客户端伪造 tenant/role/department 字段无效。
- [ ] Redis job payload 只含 ID 和文件引用，不含文件正文。
- [ ] Worker 仍无模型 Key、无 Chroma 权限、无宿主机端口。
- [ ] 上传 → 排队 → 解析 → 状态完成的 Compose 端到端测试通过。
- [ ] 删除后原文件不可访问，重复删除保持幂等。
...
- [ ] Python ruff、mypy 和 pytest 全部通过。
- [ ] 数据库迁移可在空库执行，并可在已有阶段 1 volume 上安全应用。
- [ ] Docker Compose 五服务保持 healthy。
- [ ] 至少 TXT、Markdown、DOCX、XLSX 各有一个脱敏固定 fixture 并成功解析。
- [ ] 上传、查询任务、tenant 隔离、非法文件和删除端到端测试通过。
- [ ] README、`.env.example`、OpenAPI/JSON Schema 和相关设计文档同步。
- [ ] 仓库与日志不包含密钥、上传正文、数据库文件、向量数据或本机绝对路径。
