...
> 项目：知枢 NexusKB
> 当前阶段：阶段 15——Vue 前端
> 状态：进行中（F1/F2 已完成，F3 文档生命周期与 F4 审计、Provider/系统状态、用户目录切片已完成）
...
- [x] 角色保持 OIDC 只读；身份源写回、变更审计和最后一个平台管理员保护落地前不提供误导性的本地修改。
...
- [ ] F3 多文件上传与可授权 metadata 变更等待正式 mutation 契约。
- [ ] F4 补齐角色 mutation、部门权限和用量的正式数据模型/API 后实现其余管理页面；用户目录、审计、Provider
  和系统状态已完成。
...
继续 F4，补充部门权限和用量的正式数据模型/API；角色 mutation 需先明确 OIDC 身份源写回与最后一个平台
管理员保护。F3 的
多文件上传与 metadata 修改在 mutation 契约确定后补齐。不得在缺少正式 API 时用静态假数据实现管理页面。
