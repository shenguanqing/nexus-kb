...
> 项目：知枢 NexusKB
> 当前阶段：阶段 15——Vue 前端
> 状态：进行中（F1/F2 已完成，F3 文档管理第一切片已完成）
...
- [ ] F3 继续实现入库任务页、文档详情、版本、删除和重建；多文件上传与可授权 metadata 变更等待正式 mutation 契约。
...
优先补充入库任务列表/重试契约和文档详情运行时响应契约，随后实现真实任务进度、版本、删除与重建。不得在
缺少正式 API 时用静态假数据实现管理页面。
