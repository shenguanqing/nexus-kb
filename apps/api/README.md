...
`apps/api` 是 NexusKB 的主服务，也是唯一公网业务入口。它负责认证、租户隔离、ACL、文档生命周期、异步入库、分块脱敏、Provider 编排、向量检索、回答生成、审计和指标。
...
匹配到 ACL 可见的明确文档范围后，依据不足会返回无答案，不会在 `hybrid` 模式下用通用知识猜测内部文档事实。
