...
import DocumentDangerConfirm from './DocumentDangerConfirm.vue';

const ElDialogStub = defineComponent({
  props: { modelValue: Boolean, title: String },
  template: '<section :data-open="String(modelValue)"><header>{{ title }}</header><slot /><slot name="footer" /></section>',
});
...
function mountConfirmation(action: 'cleanup' | 'delete' | 'reindex', prepared = false) {
  return mount(DocumentDangerConfirm, {
    props: {
      modelValue: true,
...
describe('DocumentDangerConfirm', () => {
  ...
  it('keeps deletion and indexing confirmations visually and semantically aligned', () => {
    ...
    expect(deletion.find('.document-danger-confirm__notice.kb-text--danger').exists()).toBe(true);
    ...
    expect(preparedIndexing.find('.document-danger-confirm__filename').text()).toBe('施工图.dwg');
  });
});
