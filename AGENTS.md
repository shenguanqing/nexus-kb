...
本项目是一套可按不同使用规模部署的知识库系统。当前目标是在 Mac 本地跑通完整 RAG 流程，后续平滑部署到 Linux 服务器。

当前仓库实现的是包含多租户、ACL、审计和管理后台的完整能力集；后续计划增加 Lite 模式，具体功能范围另行定义。Lite 必须复用同一套 RAG 核心和安全边界，通过服务端配置与 capability 收敛功能，不得仅靠前端隐藏，也不得复制出平行实现。现有 `docker:full` 表示包含 DWG 等服务的 Compose 拓扑，不代表产品版本。
...
2. `docs/01-项目实施规格.md`
3. `docs/02-技术设计.md`
4. `docs/03-前端产品与界面设计.md`
5. `docs/04-开发规范.md`
6. `docs/05-开发任务清单.md`
7. `docs/06-部署运维手册.md`
8. `docs/07-API使用说明.md`
9. `TASK.md`

`docs/README.md` 只负责阅读路径和事实源导航，不参与下列规范冲突优先级。
